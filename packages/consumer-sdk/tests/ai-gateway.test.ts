...
import { createMetricsAccumulator } from '@littlebearapps/platform-consumer-sdk';
...
describe('trackedEnv.fetch (integrated)', () => {
  ...
  it('is available on trackedEnv', async () => {
    const { withFeatureBudget } = await import('@littlebearapps/platform-consumer-sdk');
  ...
  it('tracks AI Gateway calls via trackedEnv.fetch', async () => {
    const { withFeatureBudget, getTelemetryContext } =
      await import('@littlebearapps/platform-consumer-sdk');
